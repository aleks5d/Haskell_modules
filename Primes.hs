...
updHeap :: Integral(a) => a -> BinomialHeap (a, a) -> BinomialHeap (a, a)
updHeap x heap | c == x = updHeap x $ insert dmHeap (c + d, d)
               | otherwise = heap
               where
                (c, d) = getMin heap
                dmHeap = deleteMin heap

pl :: Integral(a) => a -> BinomialHeap (a, a) -> [a]
